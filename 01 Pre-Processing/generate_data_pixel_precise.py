...
crop it pixel-precise,  (the key-function for that is np.place)
...
def generate_training_data(image_slice, mask_slice, target, bbox, padding):
    ...
    # only replace those pixels in the target
    # for which the mask pixel values == 0   
    # 1. Expand dimension
    mask_slice_3 = np.array([mask_slice, mask_slice, mask_slice])
...
# manually selected aguadas that are not cropped in any direction
# class_files = [i.split('_')[1] for i in os.listdir(root + f'data/01_raw/{cla}_sorted')]
...
for i, file in enumerate(no_classes):
    ...
    if bbox[0] == 0 or bbox[1] == 480 or bbox[2] == 0 or bbox[3] == 480:
        ...
        # remove file from the bag of images
